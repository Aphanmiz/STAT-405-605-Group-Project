...
ggplot(data=nyc_crash, aes(x = NUM_INJURED , y = BOROUGH)) +
  geom_density_ridges(aes(fill = BOROUGH), scale = 0.6) +
  scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07","green","purple","grey")) +  xlim(-1,5)

# ggplot(data=nyc_crash, aes(x = NUM_KILLED , y = BOROUGH)) +
#   geom_density_ridges(aes(fill = BOROUGH), scale = 0.6) +
#   scale_fill_manual(values = c("#00AFBB", "#E7B800", "#FC4E07","green","purple","grey")) +  xlim(-1,5)
@
